...
public class SpiralShootAbility : AbilityBase
{
    ...
    public SpiralShootAbility(string name, Sprite sprite, BulletComponent bulletPrefab, int numberOfShots, float spread, float speedVariation, float shootRate,  AbilityTargeting target = AbilityTargeting.Line)
    {
        this.name = name;
        this.sprite = sprite;
    ...
    public SpiralShootAbility(SpiralShootAbility other)
        : base(other)
    {
    ...
    {
        return other is SpiralShootAbility;
    }

    public override AbilityBase Clone()
    {
        return new SpiralShootAbility(this);
    }
    ...
    public override bool Update(AbilitySlot userSlot, float DeltaTime)
    {
        ...
        if (bulletSpawnTimer < 0)
        {
            ...
            var newRandomDirection = Rotate(direction, 360.0f * numBulletsToSpawn / (numberOfShots) );
